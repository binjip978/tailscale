...
// Viewer is a tool to automate the creation of read-only wrapper structs.
package main
...
// {{.ViewName}} provides a read-only view on {{.StructName}}.
...
{{end}}
{{define "viewSliceField"}}func (v {{.ViewName}}) {{.FieldName}}() views.ViewSlice[{{.FieldType}},{{.FieldViewName}}] { return views.SliceOfViews[{{.FieldType}},{{.FieldViewName}}](v.ж.{{.FieldName}}) }
{{end}}
...
func main() {
	...
	if err := codegen.WritePackageFile(pkg, out, it, buf); err != nil {
		log.Fatal(err)
	}
}
