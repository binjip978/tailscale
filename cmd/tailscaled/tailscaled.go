...
func main() {
	...
	flag.StringVar(&args.statepath, "state", "", "absolute path of state file; use 'kube:<secret-name>' to use Kubernetes secrets or 'arn:aws:ssm:...' to store in AWS SSM; use 'mem:' to not store state and register as an emphemeral node. If empty and --statedir is provided, the default is <statedir>/tailscaled.state")
...
func ipnServerOpts() (o ipnserver.Options) {
	...
	log.Printf("statedir: %q state: %q", o.VarRoot, statePathOrDefault())
