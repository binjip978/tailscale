...
// UserView provides a read-only view on User.
...
// NodeView provides a read-only view on Node.
// It's methods should only be called if `Valid()` returns true.
...
// HostinfoView provides a read-only view on Hostinfo.
// It's methods should only be called if `Valid()` returns true.
...
// NetInfoView provides a read-only view on NetInfo.
// It's methods should only be called if `Valid()` returns true.
...
// LoginView provides a read-only view on Login.
// It's methods should only be called if `Valid()` returns true.
...
// DNSConfigView provides a read-only view on DNSConfig.
// It's methods should only be called if `Valid()` returns true.
...
func (v DNSConfigView) Resolvers() views.ViewSlice[*dnstype.Resolver, dnstype.ResolverView] {
	return views.SliceOfViews[*dnstype.Resolver, dnstype.ResolverView](v.ж.Resolvers)
}
func (v DNSConfigView) FallbackResolvers() views.ViewSlice[*dnstype.Resolver, dnstype.ResolverView] {
...
// RegisterResponseView provides a read-only view on RegisterResponse.
// It's methods should only be called if `Valid()` returns true.
...
// DERPRegionView provides a read-only view on DERPRegion.
// It's methods should only be called if `Valid()` returns true.
...
func (v DERPRegionView) Nodes() views.ViewSlice[*DERPNode, DERPNodeView] {
...
// DERPMapView provides a read-only view on DERPMap.
// It's methods should only be called if `Valid()` returns true.
...
// DERPNodeView provides a read-only view on DERPNode.
// It's methods should only be called if `Valid()` returns true.
