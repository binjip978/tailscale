...
func HasNoClone(structTag string) bool {
	for _, keyVal := range strings.Split(structTag, " ") {
...
// Code generated by tailscale.com/util/codegen (%v); DO NOT EDIT.
...
func writeHeader(w io.Writer, pkg string) {
	exe, _ := os.Executable()

	fmt.Fprintf(w, header, time.Now().Year(), filepath.Base(exe), pkg)
}

// WritePackageFile adds a file with the provided imports and contents to package.
func WritePackageFile(pkg *packages.Package, path string, it *ImportTracker, contents *bytes.Buffer) error {
	buf := new(bytes.Buffer)
	writeHeader(buf, pkg.Name)
	it.Write(buf)
