...
// View represents the corresponding View of a Viewable. The concrete types are
...
// Viewable is any type that has had a corresponding View generated by the use
// of tailscale.com/cmd/viewer.
type Viewable[T any, V View[T]] interface {
...
func SliceOfViews[T Viewable[T, V], V View[T]](x []T) ViewSlice[T, V] {
	return ViewSlice[T, V]{x}
}

// ViewSlice is a read-only wrapper around a struct which should only be exposed
// as a View.
type ViewSlice[T Viewable[T, V], V View[T]] struct {
...
func (v ViewSlice[T, V]) MarshalJSON() ([]byte, error) { return json.Marshal(v.ж) }

// UnmarshalJSON implements json.Unmarshaler.
func (v *ViewSlice[T, V]) UnmarshalJSON(b []byte) error { return unmarshalJSON(b, &v.ж) }

// IsNil reports whether the underlying slice is nil.
func (v ViewSlice[T, V]) IsNil() bool { return v.ж == nil }

// Len returns the length of the slice.
func (v ViewSlice[T, V]) Len() int { return len(v.ж) }

// At returns a View of the element at index `i` of the slice.
func (v ViewSlice[T, V]) At(i int) V { return v.ж[i].View() }

// AppendTo appends the underlying slice values to dst.
func (v ViewSlice[T, V]) AppendTo(dst []V) []V {
	for _, x := range v.ж {
...
func (v ViewSlice[T, V]) AsSlice() []V {
